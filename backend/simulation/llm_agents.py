...
{{"new_opinion": 数值在-1到1之间(注意变化幅度限制), "reasoning": "简短理由(20字内)", "emotion": "情绪状态(冷静/愤怒/焦虑/怀疑/释然)", "action": "行动选择(转发/评论/观望/辟谣/沉默)", "is_silent": boolean(是否选择沉默), "generated_comment": "如果选择评论，生成的评论内容(30字内)", "focused_entities": ["你关注的实体名称列表"]}}
"""
...
class LLMAgent:
    ...
    async def decide_opinion(
        ...
        prompt, received_news = self.build_prompt(
            neighbor_agents, debunk_released, cocoon_strength, opinion_snapshot, knowledge_graph
        )
