...
class EnhancedMathModel:
    ...
    def _algorithmic_cocoon(
        ...
        for i in range(size):
            ...
            # 强化方向与当前观点一致
            sign = np.sign(opinions[i])

            # 强化幅度：观点越极端，强化越多
            magnitude = self.params.cocoon_strength * alpha * (1 + abs(opinions[i]))

            # 信念强度的调节作用：信念强的人更难被动摇
            belief_factor = (1 - belief_strength[i] * 0.5)

            change[i] = sign * magnitude * belief_factor
    ...
    def _spiral_of_silence(
        ...
        公式：
        if |perceived_climate - opinion| > threshold:
            isolation_fear = fear_of_isolation * gap
            silence_probability = sigmoid(isolation_fear - conviction)
        """
        size = len(opinions)
        is_silent = np.zeros(size, dtype=bool)
        ...
        for i in range(size):
            ...
            # 计算感知到的舆论气候
            neighbor_opinions = opinions[neighbors[i]]
            perceived_climate = np.mean(neighbor_opinions)

            # 观点差距
            gap = abs(perceived_climate - opinions[i])

            # 如果差距超过阈值，产生沉默压力
            if gap > self.params.silence_threshold:
                # 孤立恐惧
                isolation_fear = fear_of_isolation[i] * gap

                # 信念强度（conviction）对抗沉默
                conviction = belief_strength[i]
                ...
                if np.random.random() < silence_prob:
                    is_silent[i] = True
                    silence_pressure[i] = isolation_fear
    ...
    def _group_polarization(
        ...
        公式：
        if group_mean_opinion has same sign as own opinion:
            polarization_push = sign * polarization_factor * 0.05
        """
        size = len(opinions)
        change = np.zeros(size)
        ...
        for i in range(size):
            ...
            # 计算群体平均观点
            neighbor_opinions = opinions[neighbors[i]]
            group_mean = np.mean(neighbor_opinions)

            # 如果群体观点与个人观点同向，产生极化推力
            if np.sign(opinions[i]) == np.sign(group_mean) and opinions[i] != 0:
                # 极化方向
                direction = np.sign(opinions[i])

                # 极化幅度：群体越一致，极化越强
                group_consensus = 1 - np.std(neighbor_opinions) / 2  # 群体一致性
                magnitude = self.params.polarization_factor * 0.05 * group_consensus

                # 信念强度抑制极化
                magnitude *= (1 - belief_strength[i] * 0.3)

                change[i] = direction * magnitude
    ...
    def _debunk_with_backfire(
        ...
        公式：
        if belief_strength > backfire_threshold:
            backfire_risk = backfire_strength * (belief_strength - threshold)
            if random < backfire_risk:
                opinion moves opposite to debunk direction
        """
        size = len(opinions)
        change = np.zeros(size)
        ...
        for i in range(size):
            ...
            # 计算个体层面的效果
            individual_effect = effectiveness * (1 - belief_strength[i] * 0.5)
            individual_effect *= (0.5 + susceptibility[i] * 0.5)

            # 逆火效应检测
            # 信念极强的人可能产生逆火
            backfire_threshold = 0.7
            if belief_strength[i] > backfire_threshold:
                backfire_risk = self.params.backfire_strength * (belief_strength[i] - backfire_threshold)
                if np.random.random() < backfire_risk:
                    # 逆火：观点向相反方向移动（更相信谣言）
                    change[i] = -individual_effect * 0.5
                    logger.debug(f"Agent {i}: Backfire effect triggered")
                else:
                    # 正常辟谣效果
                    change[i] = individual_effect
            else:
                # 正常辟谣效果
                change[i] = individual_effect
    ...
    def _cognitive_dissonance(
        ...
        """
        size = len(opinions)
        adjustment = np.zeros(size)

        for i in range(size):
            change_magnitude = abs(total_change[i])

            # 变化幅度超过阈值时触发认知失调
            if change_magnitude > self.params.cognitive_dissonance_threshold:
                # 计算失调程度
                dissonance = change_magnitude - self.params.cognitive_dissonance_threshold

                # 信念越强，越可能强化而非改变
                reinforce_probability = belief_strength[i] * 0.5

                if np.random.random() < reinforce_probability:
                    # 强化原有信念，抵消部分变化
                    adjustment[i] = -total_change[i] * dissonance * 0.5
