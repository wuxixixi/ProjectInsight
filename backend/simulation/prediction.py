...
class PredictionInterval:
    """预测区间"""
    expected: float      # 期望值
    optimistic: float    # 乐观值（下界，对谣言传播）
    pessimistic: float   # 悲观值（上界，对谣言传播）
...
class MonteCarloPredictor:
    ...
    def _estimate_parameters(self):
        ...
        # 提取时间序列
        rumor_rates = [h.get("rumor_spread_rate", 0) for h in self.history]
        truth_rates = [h.get("truth_acceptance_rate", 0) for h in self.history]
        ...
        self.trend_estimate = {
            "rumor_spread_rate": calc_trend(rumor_rates),
            "truth_acceptance_rate": calc_trend(truth_rates),
        ...
        self.volatility_estimate = {
            "rumor_spread_rate": calc_volatility(rumor_rates),
            "truth_acceptance_rate": calc_volatility(truth_rates),
    ...
    def predict(self, current_state: Dict) -> Dict[str, PredictionInterval]:
        ...
        Args:
            current_state: 当前状态 {rumor_spread_rate, truth_acceptance_rate, polarization_index, ...}
        ...
        # 运行蒙特卡洛模拟
        metrics = ["rumor_spread_rate", "truth_acceptance_rate", "polarization_index"]
        predictions = {}
    ...
    def _fallback_prediction(self, current_state: Dict) -> Dict[str, PredictionInterval]:
        ...
        for metric in ["rumor_spread_rate", "truth_acceptance_rate", "polarization_index"]:
...
class PredictionModel:
        ...
        Returns:
            {
                ...
                "truth_acceptance_rate": PredictionInterval,
    ...
    def get_intervention_recommendation(
        ...
        """
        rumor_prediction = prediction.get("rumor_spread_rate", {})
        truth_prediction = prediction.get("truth_acceptance_rate", {})
        polarization_prediction = prediction.get("polarization_index", {})

        expected_rumor = rumor_prediction.get("expected", 0.5)
        pessimistic_rumor = rumor_prediction.get("pessimistic", 0.5)
        expected_truth = truth_prediction.get("expected", 0.3)
        expected_polarization = polarization_prediction.get("expected", 0.3)

        # 当前状态
        current_rumor = current_state.get("rumor_spread_rate", 0.3)
        ...
        if pessimistic_rumor > 0.7:
            risk_level = "critical"
            risk_factors.append("谣言传播率预测超70%")
        elif pessimistic_rumor > 0.5:
            risk_level = "high"
            risk_factors.append("谣言传播率预测超50%")
        elif pessimistic_rumor > 0.35:
            risk_level = "medium"
            risk_factors.append("谣言传播率呈上升趋势")
        ...
        # 计算最佳干预时机
        # 基于谣言传播速率（预期增长率）
        if current_rumor < 0.2:
            best_timing = 5  # 早期，可稍作观察
        elif current_rumor < 0.4:
        ...
        strategies = self._generate_strategies(
            risk_level, current_rumor, expected_truth, expected_polarization
        )

        # 分析窗口收窄情况
        window_status = "充足"
        if current_rumor > 0.4:
            window_status = "收窄"
        if current_rumor > 0.6:
            window_status = "紧迫"

        # 决策依据
        reasoning = self._generate_reasoning(
            current_rumor, expected_rumor, expected_polarization, risk_factors
        )
        ...
        recommendation = {
            ...
            "metrics": {
                "current_rumor_rate": current_rumor,
                "expected_rumor_rate": expected_rumor,
                "expected_truth_rate": expected_truth,
                "expected_polarization": expected_polarization
            },
            "message": self._generate_message(risk_level, expected_rumor, window_status),
            "reasoning": reasoning
        }
    ...
    def _generate_strategies(
        ...
        strategies.append({
            "type": "debunk",
            "name": "权威辟谣",
            "description": "通过官方渠道发布辟谣信息",
            "effectiveness": 0.8 if risk_level in ["high", "critical"] else 0.6,
            "cost": "high"
        })

        # 谣言传播早期：预防性沟通
        if current_rumor < 0.3:
            strategies.append({
                "type": "prevent",
                "name": "预防性科普",
                "description": "在谣言大规模传播前进行科普教育",
                "effectiveness": 0.7,
                "cost": "medium"
            })

        # 真相信仰率低：增强真相传播
        if expected_truth < 0.3:
            strategies.append({
                "type": "amplify",
                "name": "真相放大",
                "description": "通过意见领袖和权威媒体放大真相传播",
                "effectiveness": 0.65,
                "cost": "medium"
            })
    ...
    def _generate_reasoning(
        self,
        current_rumor: float,
        expected_rumor: float,
        polarization: float,
        risk_factors: List[str]
    ) -> str:
        """生成决策依据说明"""
        trend = "上升" if expected_rumor > current_rumor else "稳定" if expected_rumor == current_rumor else "下降"

        reasoning = f"当前谣言传播率{current_rumor*100:.0f}%，预测{trend}趋势（{expected_rumor*100:.0f}%）。"

        if risk_factors:
            reasoning += f"风险因素：{'；'.join(risk_factors)}。"

        if polarization > 0.5:
            reasoning += "群体存在极化倾向，单一辟谣效果可能受限。"

        return reasoning

    def _generate_message(self, risk_level: str, expected_rumor: float, window_status: str) -> str:
        """生成干预建议消息"""
        if risk_level == "critical":
            return f"🚨 临界风险：预计谣言传播率达{expected_rumor*100:.0f}%，干预窗口{window_status}，建议立即多渠道联动干预"
        elif risk_level == "high":
            return f"🚨 高风险：预计谣言传播率达{expected_rumor*100:.0f}%，建议立即加强权威辟谣"
        elif risk_level == "medium":
            return f"⚠️ 中风险：预计谣言传播率{expected_rumor*100:.0f}%，建议适时介入，优先投放权威信息"
        else:
