...
LLM Agent 在决策时会接收知识图谱作为上下文：

```
当前事件知识图谱：
- 实体: 王某杞(人物), 某科技公司(组织), 北京(地点), 100亿元(概念)
- 关系: 王某杞-担任CEO->某科技公司, 某科技公司-计划投资->100亿元
- 摘要: 某科技公司CEO在北京宣布投资100亿元
...
1. **解析新闻**：在前端点击"解析新闻"按钮，预览知识图谱结构
2. **注入事件**：在推演过程中注入新事件，自动触发知识图谱解析
3. **图谱可视化**：查看完整知识图谱的实体和关系网络
